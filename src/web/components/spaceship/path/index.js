...
const drawX = spaceship => element => {
  const { coordinate, destination } = spaceship.getState()
...
const curriedDrawPath = spaceship => element => {
  const { coordinate, destination, size } = spaceship.getState()
  ...
  const drawPath = (x = coordinate.x(), y = coordinate.y()) => (
    lastPosition = null,
    shouldDraw = true
  ) => {
    ...
    if (distance > 5) {
      ...
      if (shouldDraw) {
        lineTo(Coordinate(x - coordinate.x(), y - coordinate.y()))(element)
      }
      return drawPathAtPosition(null, !shouldDraw)
    }
...
const setStart = spaceship => element => {
  ...
  const { coordinate } = spaceship.getState()
  return compose(
    moveTo(coordinate),
