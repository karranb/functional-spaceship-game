...
export const reduce = ([x, ...xs], f, memo, i = 0) => def(x)
...
export const filter = (xs, fn) => reduce(xs, (memo, x) => fn(x)
    ? [...memo, x] : [...memo], [])

export const reject = (xs, fn) => reduce(xs, (memo, x) => fn(x)
    ? [...memo] : [...memo, x], [])

// export const first = (xs, n) => reduce(xs, (memo, x, i) => i < n
//     ? [...memo, x] : [...memo], [])

// export const last = (xs, n) => reduce(xs, (memo, x, i) => i >= (length(xs) - n)
//     ? [...memo, x] : [...memo], [])

export const merge = spreadArg(xs => reduce(xs, (memo, x) => [...memo, ...x], []))

export const flatten = xs => reduce(xs, (memo, x) => x
    ? isArray(x) ? [...memo, ...flatten(x)] : [...memo, x] : [], [])

// export const add = spreadArg(([x, ...xs]) => reduce(xs, (memo, y) => memo + y, x))

// export const divide = spreadArg(([x, ...xs]) => reduce(xs, (memo, y) => memo / y, x))

// export const multiply = spreadArg(([x, ...xs]) => reduce(xs, (memo, y) => memo * y, x))

export const flow = (...args) => init => reduce(args, (memo, fn) => fn(memo), init)

export const compose = (...args) => flow(...reverse(args))

export const anyCompose = (...fns) =>
  reduce(fns, (memo, fn) => memo || fn(), false)

export const everyCompose = (...fns) =>
  reduce(fns, (memo, fn) => memo && fn(), true)


export const diff = arr1 => arr2 => arr1.filter(item => arr2.indexOf(item) < 0)

// export const anyPass = (...args) => 